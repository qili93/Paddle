...
function(cc_test_build TARGET_NAME)
  if(WITH_TESTING)
    ...
    get_property(os_dependency_modules GLOBAL PROPERTY OS_DEPENDENCY_MODULES)
    # ROCM 3.5 do not support static link, ROCM 4.0 has fixed this issue
    # Refer to: https://github.com/RadeonOpenCompute/ROCm/issues/1189
    target_link_libraries(${TARGET_NAME} ${cc_test_DEPS} ${os_dependency_modules} ${ROCM_HIPRTC_LIB} paddle_gtest_main lod_tensor memory gtest gflags glog)
  ...
  if (WITH_ROCM)
    ...
    if(hip_library_SRCS)
      ...
      if(NOT ${CMAKE_CURRENT_SOURCE_DIR} MATCHES ".*/operators")
        ...
        # message(STATUS "hip_library_SRCS=${hip_library_SRCS}")
        # message(STATUS "CMAKE_CURRENT_SOURCE_DIR=${CMAKE_CURRENT_SOURCE_DIR}")
      endif()
      # set(hip_library_cu_srcs)
      # foreach(src ${hip_library_SRCS})
      #   if(${src} MATCHES "\\.cu$")
      #     list(APPEND hip_library_cu_srcs ${src})
      #   endif()
      # endforeach()
      # list(LENGTH hip_library_cu_srcs hip_library_cu_srcs_len)
      # if (${hip_library_cu_srcs_len} EQUAL 0)
      #   message(STATUS "TARGET_NAME=${TARGET_NAME}")
      #   message(STATUS "hip_library_SRCS=${hip_library_SRCS}")
      #   message(STATUS "CMAKE_CURRENT_SOURCE_DIR=${CMAKE_CURRENT_SOURCE_DIR}")
      ...
      #   if(NOT ${CMAKE_CURRENT_SOURCE_DIR} MATCHES "\\operators$")
      #     message(STATUS "===  This is under operators folder====")
      #   endif()
      #   set_source_files_properties(${hip_library_SRCS} PROPERTIES HIP_SOURCE_PROPERTY_FORMAT 1)
      # endif()
...
function(hip_binary TARGET_NAME)
  if (WITH_ROCM)
    ...
    # set_source_files_properties(${hip_binary_SRCS} PROPERTIES HIP_SOURCE_PROPERTY_FORMAT 1)
...
function(hip_test TARGET_NAME)
  ...
  if (WITH_ROCM AND WITH_TESTING AND NOT "$ENV{CI_SKIP_CPP_TEST}" STREQUAL "ON")
    ...
    # FindHIP.cmake defined hip_add_executable, HIP_SOURCE_PROPERTY_FORMAT is requried for .cc files
    # set_source_files_properties(${hip_test_SRCS} PROPERTIES HIP_SOURCE_PROPERTY_FORMAT 1)
    ...
    get_property(os_dependency_modules GLOBAL PROPERTY OS_DEPENDENCY_MODULES)
    target_link_libraries(${TARGET_NAME} ${hip_test_DEPS} paddle_gtest_main lod_tensor memory gtest gflags glog ${os_dependency_modules} ${ROCM_HIPRTC_LIB})
