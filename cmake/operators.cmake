...
function(op_library TARGET)
    ...
    if (${op_library_SRCS_len} EQUAL 0)
        ...
        if(WITH_ROCM_PLATFORM)
            ...
            string(REPLACE "_op" "_cudnn_op" HIPDNN_FILE "${TARGET}")
            if (EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${HIPDNN_FILE}.cu.cc)
                list(APPEND hipdnn_cu_cc_srcs ${HIPDNN_FILE}.cu.cc)
            endif()
            if (EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${HIPDNN_FILE}.cu)
                list(APPEND hipdnn_cu_srcs ${HIPDNN_FILE}.cu)
            endif()
        endif()
        if(WITH_MKLDNN)
        ...
        foreach(src ${op_library_SRCS})
            if(WITH_ROCM_PLATFORM AND ${src} MATCHES ".*_cudnn_op.cu$")
                list(APPEND hipdnn_cu_srcs ${src})
            elseif(WITH_ROCM_PLATFORM AND ${src} MATCHES ".*\\.cu$")
                list(APPEND hip_srcs ${src})
            elseif(WITH_ROCM_PLATFORM AND ${src} MATCHES ".*_cudnn_op.cu.cc$")
                list(APPEND hipdnn_cu_cc_srcs ${src})
            elseif(WITH_ROCM_PLATFORM AND ${src} MATCHES ".*\\.cu.cc$")
    ...
    elseif (WITH_ROCM_PLATFORM)
        hip_library_ops(${TARGET} SRCS ${cc_srcs} ${hip_cc_srcs} ${hipdnn_cu_cc_srcs} ${hipdnn_cu_srcs} ${mkldnn_cc_srcs} ${hip_srcs} DEPS ${op_library_DEPS}
                ${op_common_deps})
    else()
        # Unity Build relies on global option `WITH_UNITY_BUILD` and local option `UNITY`.
        if(WITH_UNITY_BUILD AND op_library_UNITY)
...
"fused_bn_add_activation_op")
    ...
    list(LENGTH hipdnn_cu_cc_srcs hipdnn_cu_cc_srcs_len)
    if (${pybind_flag} EQUAL 0 AND ${mkldnn_cc_srcs_len} EQUAL 0 AND ${cu_srcs_len} EQUAL 0 AND ${cu_cc_srcs_len} EQUAL 0 AND
        ${hip_cu_srcs_len} EQUAL 0 AND ${hipdnn_cu_cc_srcs_len} EQUAL 0 AND ${xpu_cc_srcs_len} EQUAL 0)
    ...
    # pybind USE_OP_DEVICE_KERNEL for HIPDNN
    list(LENGTH hipdnn_hip_cc_srcs hipdnn_hip_cc_srcs_len)
    if (WITH_ROCM_PLATFORM AND ${hipdnn_hip_cc_srcs_len} GREATER 0)
      if(${TARGET} STREQUAL "activation")
        file(APPEND ${pybind_file} "USE_OP_DEVICE_KERNEL(relu, CUDNN);\n")
      else()
       file(APPEND ${pybind_file} "USE_OP_DEVICE_KERNEL(${TARGET}, CUDNN);\n")
    ...
    # pybind USE_OP_DEVICE_KERNEL for HIPDNN
    list(LENGTH hipdnn_cu_srcs hipdnn_cu_srcs_len)
    if (WITH_ROCM_PLATFORM AND ${hipdnn_cu_srcs_len} GREATER 0)
       file(APPEND ${pybind_file} "USE_OP_DEVICE_KERNEL(${TARGET}, CUDNN);\n")
    endif()
