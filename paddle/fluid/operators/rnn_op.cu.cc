...
class RNNDescriptors {
 public:
  RNNDescriptors(int seq_length, int batch_size, int input_size,
                 ...
                 int weight_numel, gpuDnnRNNMode_t mode, bool is_bidirec,
                 bool is_test)
...
// ------------------- cudnn rnn descriptors ---------------------
    ...
    PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::hipdnnSetRNNDescriptor(
        ...
        is_bidirec_ ? GPUDNN_BIDIRECTIONAL : GPUDNN_UNIDIRECTIONAL, mode_,
...
class RNNCudnnKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    gpuDnnRNNMode_t rnn_mode = GPUDNN_LSTM;
    if (mode == "LSTM")
      rnn_mode = GPUDNN_LSTM;
    else if (mode == "GRU")
      rnn_mode = GPUDNN_GRU;
    else if (mode == "RNN_RELU")
      rnn_mode = GPUDNN_RNN_RELU;
    else if (mode == "RNN_TANH")
      rnn_mode = GPUDNN_RNN_TANH;
    else
...
#ifdef PADDLE_WITH_HIP
        PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::hipdnnRNNForwardTraining(
  ...
  void RNNInferece(const bool &has_seq_length, const gpuDnnHandle_t &handle,
    ...
    if (!has_seq_length) {
      ...
      // This interface is used when the input/output is unpadded.
#ifdef PADDLE_WITH_HIP
      PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::hipdnnRNNForwardInference(
          handle, rnn->rnn_desc(), seq_length, rnn->x_descs(), x_data,
          rnn->init_h_desc(), init_h_data, rnn->init_c_desc(), init_c_data,
          rnn->weight_desc(), w_data, rnn->y_descs(), out_data,
          rnn->last_h_desc(), last_h_data, rnn->last_c_desc(), last_c_data,
...
class RNNGradCudnnKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    if (!has_seq_length) {
#ifdef PADDLE_WITH_HIP
      PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::hipdnnRNNBackwardData(
      ...
      PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::hipdnnRNNBackwardWeights(
...
REGISTER_OP_CUDA_KERNEL(rnn, 
#ifndef PADDLE_WITH_HIP
                        ops::RNNCudnnKernel<double>,
#endif
                        ops::RNNCudnnKernel<float>);
REGISTER_OP_CUDA_KERNEL(rnn_grad, 
#ifndef PADDLE_WITH_HIP
                        ops::RNNGradCudnnKernel<double>,
#endif
                        ops::RNNGradCudnnKernel<float>);
