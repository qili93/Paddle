...
static void feed_tensor_data(const platform::DeviceContext& ctx, 
  ...
  tensor->Resize(dims);
  print_data_2d(data.data(), numel, framework::vectorize(dims), name);
}

// input
const std::vector<int64_t> input_dim_vec = {2, 3};
const int64_t input_numel = std::accumulate(input_dim_vec.begin(), input_dim_vec.end(), 1, std::multiplies<int64_t>());
const std::vector<int64_t> label_dim_vec = {2, 1};
const int64_t label_numel = std::accumulate(label_dim_vec.begin(), label_dim_vec.end(), 1, std::multiplies<int64_t>());
// attrs
...
void TestCrossEntropy(const platform::DeviceContext& ctx, 
  ...
  auto softmax_tensor = scope.Var("Softmax")->GetMutable<framework::LoDTensor>();
  auto loss_tensor = scope.Var("Loss")->GetMutable<framework::LoDTensor>();

  // feed input data
  feed_tensor_data<T>(ctx, input_dims, logits_tensor, false, "logit");
  feed_tensor_data<int64_t>(ctx, label_dims, label_tensor, false, "label");
  ...
  desc_bwd.SetInput(framework::GradVarName("Loss"), {framework::GradVarName("Loss")});
  desc_bwd.SetOutput(framework::GradVarName("Logits"), {framework::GradVarName("Logits")});
  ...
  auto loss_grad_tensor = scope.Var(framework::GradVarName("Loss"))->GetMutable<framework::LoDTensor>();
  auto logit_grad_tensor = scope.Var(framework::GradVarName("Logits"))->GetMutable<framework::LoDTensor>();

  // feed loss_grad_tensor data
  feed_tensor_data<T>(ctx, loss_tensor->dims(), loss_grad_tensor, true, "loss_grad");
...
static void compare_results(const std::vector<T> cpu_out, 
  ...
  if (!result) {
    ...
    print_data_2d(cpu_out.data(), numel, dims, name + "_cpu");
    print_data_2d(gpu_out.data(), numel, dims, name + "_gpu");
  } else {
...
TEST(test_conv2d_op, compare_cpu_and_gpu) {
  ...
  compare_results<float>(softmax_cpu, softmax_gpu, input_numel, input_dim_vec, "softmax");
  compare_results<float>(loss_cpu, loss_gpu, label_numel, label_dim_vec, "loss");
  compare_results<float>(logit_grad_cpu, logit_grad_gpu, input_numel, input_dim_vec, "logit_grad");
}
