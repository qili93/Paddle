...
const std::vector<int64_t> y_dims = {3, 1, 2};
...
void TestMain(const platform::DeviceContext& ctx,
  ...
  feed_ones<T>(ctx, x_ddims, x0_tensor, "x0");
  feed_ones<T>(ctx, x_ddims, x1_tensor, "x1");
  feed_ones<T>(ctx, x_ddims, x2_tensor, "x2");
...
static void compare_results(const std::vector<T> cpu_data,
                            const std::vector<T> gpu_data, 
                            const int64_t data_numel,
                            const std::string name) {
  auto result = std::equal(
      cpu_data.begin(), cpu_data.end(), gpu_data.begin(),
  ...
  if (!result) {
    ...
    print_data(gpu_data.data(), data_numel, name + "_gpu");
  } else {
...
TEST(test_stack_op, compare_cpu_and_gpu) {
  ...
  std::vector<float> gpu_y_data(y_numel);
  std::vector<float> gpu_x0_grad_data(x_numel);
  std::vector<float> gpu_x1_grad_data(x_numel);
  std::vector<float> gpu_x2_grad_data(x_numel);
  TestMain<float>(gpu_ctx, gpu_y_data, gpu_x0_grad_data, gpu_x1_grad_data, gpu_x2_grad_data);

  compare_results<float>(cpu_y_data, gpu_y_data, y_numel, "Y");
  compare_results<float>(cpu_x0_grad_data, gpu_x0_grad_data, x_numel, "x0_grad");
  compare_results<float>(cpu_x1_grad_data, gpu_x1_grad_data, x_numel, "x1_grad");
  compare_results<float>(cpu_x2_grad_data, gpu_x2_grad_data, x_numel, "x2_grad");
}
