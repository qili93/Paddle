...
class StackNPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    PADDLE_ENFORCE_GT(
        num, 0, platform::errors::InvalidArgument("number of input Tensor <= 0"));
    ...
    const auto& runner = NpuOpRunner("Pack", {x_list}, {*y}, {{"axis", axis}, {"N", num}});
    runner.Run(stream);
  }
};
...
class StackGradNPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    const auto& runner = NpuOpRunner("Unpack", {*dy}, {dx_list}, {{"axis", axis}, {"num", num}});
    runner.Run(stream);
  }
};
...
REGISTER_OP_NPU_KERNEL(
    ...
    ops::StackNPUKernel<paddle::platform::NPUDeviceContext, paddle::platform::float16>);

REGISTER_OP_NPU_KERNEL(
    stack_grad, ops::StackGradNPUKernel<paddle::platform::NPUDeviceContext, float>,
    ops::StackGradNPUKernel<paddle::platform::NPUDeviceContext, paddle::platform::float16>);
