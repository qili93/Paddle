...
template <typename T>
inline gpuDnnDataType_t ToMIOpenDataType(const T& t) {
...
class TensorDescriptor {
 public:
  TensorDescriptor() {
    PADDLE_ENFORCE_CUDA_SUCCESS(
        dynload::miopenCreateTensorDescriptor(&desc_));
  }
  ~TensorDescriptor() {
    PADDLE_ENFORCE_CUDA_SUCCESS(
        dynload::miopenDestroyTensorDescriptor(desc_));
  }
  ...
  void set(const Tensor& tensor, const int groups = 1) {
    ...
    PADDLE_ENFORCE_CUDA_SUCCESS(dynload::miopenSetTensorDescriptor(
        desc_, ToMIOpenDataType(tensor.type()), static_cast<int>(dims_with_group.size()),
        const_cast<int*>(dims_with_group.data()), const_cast<int*>(strides.data())));
  }
...
class FilterDescriptor {
 public:
  ...
  void set(const Tensor& tensor, const miopenTensorFormat_t format, const int groups = 1) {
    auto dims = framework::vectorize<int>(tensor.dims());
    std::vector<int> transformed_dims;
    if (format == MIOPEN_TENSOR_NHWC) {
    ...
    // std::vector<int> strides(transformed_dims.size());
    // strides[transformed_dims.size() - 1] = 1;
    // for (int i = transformed_dims.size() - 2; i >= 0; i--) {
    //   strides[i] = transformed_dims[i + 1] * strides[i + 1];
    // }
    PADDLE_ENFORCE_CUDA_SUCCESS(dynload::miopenSetTensorDescriptor(
        desc_, ToMIOpenDataType(tensor.type()), static_cast<int>(transformed_dims.size()), 
 ...
 public:
  ...
  void set(miopenDataType_t dtype, const std::vector<int>& pads,
    ...
    PADDLE_ENFORCE_CUDA_SUCCESS(dynload::miopenInitConvolutionNdDescriptor(
        desc_, static_cast<int>(pads.size()), 
        const_cast<int*>(pads.data()), 
        const_cast<int*>(strides.data()), 
        const_cast<int*>(dilations.data()),
        miopenConvolution));
    PADDLE_ENFORCE_CUDA_SUCCESS(platform::dynload::miopenSetConvolutionGroupCount(desc_, groups));
  }
